...
class MyMultiSuggestModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl).addButton((btn) =>
			btn.setButtonText("Clear all").onClick(() => {
				let toggles = document.querySelectorAll(
					".select-line .filter-checkbox.is-enabled"
		...
		for (let val of this.values) {
			...
			new Setting(contentEl)
				...
				.addToggle((toggle) => {
					...
					toggle.onChange((toggleValue) => {
						...
						let excludeOn: HTMLInputElement | null =
							document.querySelector(
								"." +
									lineClass +
									" .filter-checkbox:nth-child(2).is-enabled"
							);
				...
				.addToggle((toggle) => {
					...
					toggle.onChange((toggleValue) => {
						...
						let includeOn: HTMLInputElement | null =
							document.querySelector(
								"." +
									lineClass +
									" .filter-checkbox:nth-child(1).is-enabled"
							);
			...
			let includeCheckbox = document.querySelector(
				"." + lineClass + " .checkbox-container:nth-child(1)"
			);
			includeCheckbox!.classList.add("include");
			includeCheckbox!.classList.add("filter-checkbox");
			let excludeCheckbox = document.querySelector(
				"." + lineClass + " .checkbox-container:nth-child(2)"
			);
			...
			btn
				...
				.onClick(() => {
					...
					let inputEl: HTMLInputElement | null =
						contentEl.querySelector(".number-input-el");
...
class API {
	...
	async dateFilterInput(name: string, defaultVal: string) {
		...
		let selection = await this.suggester([
			"-all-",
			"-",
			"day",
			"week",
		...
		if (selection == "day") {
			let daySelection = await this.suggester([
				"today",
				"tomorrow",
				"yesterday",
				"date",
			...
			let period: any =
				(await this.datePeriodInput(name, defaultVal)) || "";
			if (period != "") {
				dateString =
					"period:start:" + period.start + ":end:" + period.end;
			}
		} else if (selection == "week") {
			let weekSelection = await this.suggester([
				"current week",
				"next week",
				"last week",
				"select",
			...
			if (weekSelection == "select") {
				...
				let week = await this.selectSuggester(
					"Week",
					weekValues,
					weekNames,
					currentWeekString
			...
			let monthSelection = await this.suggester([
				"current month",
				"next month",
				"last month",
				"select",
			...
			if (monthSelection == "select") {
				...
				let month = await this.selectSuggester(
					"Month",
					monthValues,
					monthNames,
					currentMonthVal
			...
			let yearSelection = await this.suggester([
				"current year",
				"next year",
				"last year",
				"select",
			...
			if (yearSelection == "select") {
				...
				let year = await this.selectSuggester(
					"Year",
					yearArr,
					yearArr,
					currentYear + ""
	...
	getPropType(prop: string) {
		...
		if (
			...
			type = "multitext";
		} else if (prop == "file.cday" || prop == "file.mday") {
			type = "date";
		} else if (prop == "file.ctime" || prop == "file.mtime") {
			type = "datetime";
	...
	listContainsPath(prop: any, text: string) {
		if (
			prop &&
			Array.isArray(prop) &&
			prop.find((p) => p.path && p.path == text)
		) {
			return true;
			...
			for (let property of prop) {
				if (
					!property.path.match(".md") &&
					(text == property.path + ".md" ||
						text.match("/" + property.path + ".md"))
				) {
					return true;
				}
			}
	...
	getValueNames(values: string[], filter: any) {
		let valueNames = values.map((v) => {
			...
			if (
				...
				(v.startsWith("[[") &&
					filter &&
					filter.path == this.getWikilinkPath(v))
			) {
				v = v + " ✔";
			}
			return v;
	...
	async filterButton(
		...
		if (
			current[propName] &&
			current[propName] != "all" &&
			current[propName].length != 0
		) {
		...
		if (multiSelect) {
			...
			if (
				(current[propNameInclude] &&
					current[propNameInclude].length != 0) ||
				(current[propNameExclude] &&
					current[propNameExclude].length != 0)
			) {
				buttonClass = "dvit-button button-selected";
			}
		}
			...
			for (let key of keyWords) {
				if (key.length > 0) {
					pages = pages.filter((page) =>
						page.file.name.toLowerCase().includes(key.toLowerCase())
					);
				}
			}
		}
	...
	async filter(p: PropObject, filteredPages: any[], id: string) {
		let { dv, isLink, containsPath, listContainsPath, fixList, getVal } =
			this;
		...
		if (propType == "text" || propType == "number") {
			if (filter == "-") {
				...
				return filteredPages.filter((p) =>
					containsPath(getVal(p, prop), filter.path)
				);
			} else if (filter && filter != "all") {
				if (prop == "file.folder") {
					return filteredPages.filter((p) =>
						getVal(p, prop).startsWith(filter)
					);
				} else
					return filteredPages.filter(
						(p) => getVal(p, prop) == filter
					);
			} else {
		...
		if (propType == "multitext") {
			if (multiSelect) {
				...
				if (filter == "-") {
					filteredPages = filteredPages.filter(
						(p) => !getVal(p, prop) || getVal(p, prop).length == 0
					);
				} else if (isLink(filter)) {
					filteredPages = filteredPages.filter((p) => {
						let val = fixList(getVal(p, prop));
		...
		if (propType == "checkbox") {
			if (filter == "-") {
				return filteredPages.filter(
					(p) => getVal(p, prop) === undefined
				);
			} else if (filter === false) {
				return filteredPages.filter(
					(p) => getVal(p, prop) === false || getVal(p, prop) === null
				);
	...
	async changeProp(p: PropObject, pages: any[], id: string) {
		...
		if (propType == "text") {
			...
			let val = (await this.dateFilterInput("Date", "")) || "";
			...
			let newFilter: MultiFilter = await this.multiSuggestDouble(
				prop,
				valueNames,
				multiValues,
				filter
			...
			if (
				(!newFilter.include || newFilter.include.length == 0) &&
	...
	async newEntryButton(
		args: { noteName: string; noteTemplate: string; noteFolder: string },
		container: HTMLElement
	) {
		...
		const createNote = async (openMode: string) => {
			let template = this.app.vault
				.getFiles()
				.find((f) => f.basename == noteTemplate);
			let data = "";
			if (template) {
	...
	taskProgress(p: any) {
		...
		if (max > 0) {
			return (
				"<div class='task-progress'><progress max=" +
				max +
				" value=" +
				value +
				"></progress> <span>" +
	...
	async createTable(
						...
						tableProps.map(async (propItem) => {
									...
									if (p.path) {
										...
										propItem.replace!.forEach((r) => {
											if (r[2] == "icon") {
												...
												if (iconEl)
													r[1] =
														iconEl.outerHTML.replace(
															"svg-icon",
															"svg-icon dv-tag dv-tag-" +
																r[0]
														);
											}
							...
							if (
								...
								if (propVal.path) {
									...
									if (!display) {
										display = propVal.path.replace(
											/(.*?)([^\/]*)(\.md)/,
											"$2"
										);
									}
									propVal.display = display.slice(
										...propItem.slice
									);
								} else {
							...
							if (!propName.startsWith("file.")) {
								...
								if (propType == "date") {
									...
									if (propVal) {
										let dateFormat =
											Dataview.getAPI().settings
												.defaultDateFormat;
										let locale =
											localStorage.getItem("language");
										propVal = propVal.toFormat(dateFormat, {
											locale: locale,
										});
									}
								}
							...
							if (
								...
								editButton.innerHTML == "undefined" ||
								editButton.innerHTML == "<ul></ul>"
							) {
								editButton.innerHTML = propName;
								if (propItem.name)
									editButton.innerHTML = propItem.name;
								if (propItem.icon) {
									let iconWrapper =
							...
							if (propType == "checkbox") {
								propVal =
									'<input type="checkbox" class="prop-checkbox" data-path="' +
									p.file.path +
									'" data-prop="' +
									propName +
									'">';
							...
							if (propName == "slider") {
								...
								if (max) {
									let slider =
										...
										p[propMax] +
										' step="1" class="slider table-property-slider" data-path="' +
										p.file.path +
										'" data-prop="' +
										propSliderVal +
										'" data-prop="' +
							...
							if (propName == "file.tasks") {
								...
								if (p.file && p.file.tasks) {
									...
									taskList.className =
										"contains-task-list has-list-bullet";
									for (let task of tasks) {
										let taskLine =
											document.createElement("li");

										taskLine.classList.add(
											"task-list-item"
										);

										taskLine.setAttribute(
											"data-task",
											task.status
										);
		...
		if (cardsView) {
			...
			if (cardsView.cardsWidth) {
				tableWrapper.classList.add(
					"cards-width-" + cardsView.cardsWidth
				);
			}
		}
		...
		for (let c of taskCheckboxes) {
			...
			checkbox.onchange = async () => {
				let path = checkbox.getAttribute("data-path") ?? "";
				let lineNum: any =
					checkbox.getAttribute("data-task-line") ?? "";
				let file = this.app.vault.getAbstractFileByPath(path) as TFile;
				let content = await this.app.vault.cachedRead(file);
				let lines = content.split("\n");
				let line = lines[lineNum];
		...
		let excerpts = document.querySelectorAll(
			".edit-button[data-type='excerpt']"
		);
	...
	updateSlider(slider: HTMLInputElement) {
		...
		slider.style.backgroundImage =
			"linear-gradient(" + imageColor + ", " + imageColor + ")";
	...
	async editProp(type: string, path: string, prop: string) {
		...
		if (this.isLink(prevVal)) {
			prevVal = "[[" + this.getDisplay(prevVal) + "]]";
		}
		...
		if (type == "text") {
			...
			if (propItem && propItem.valueOptions) {
				let options = propItem.valueOptions;
				values = [...options];
			} else {
				console.error(
					"propItem is undefined or does not have valueOptions"
				);
			}
	...
	async renderView(
		...
		let viewContainer = container.createEl("div", {
			cls: "dvit-view-id-" + id,
		});
		...
		if (!view || view == "table") {
			...
			await this.createTable(
				...
				id,
				fullWidth,
				{ cards: true, position: cardsPosition, cardsWidth }
			);
		} else if (view == "list") {
		...
		let search = buttonPanelTop.querySelector(
			".dvit-search-input"
		) as HTMLInputElement;
		if (search) {
			search.focus();
	...
	async renderGallery(dv: any, paginationNum: number) {
		...
		for (let file of imageFiles) {
			...
			imageblock =
				imageblock + "<span class='gallery-image'>" + image + "</span>";
		}
	...
	async renderList(settings: any, list: any, dv: any) {
		...
		if (group) {
			list = list
				.groupBy((l: any) => l[group])
				.sort(
					(g: any) =>
						window
							.moment(g.key.subpath, "DD MMMM YYYY — dddd")
							.unix(),
					"desc"
				);
			for (let l of list) {
